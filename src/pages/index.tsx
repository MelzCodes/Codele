...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchActiveProblemId = async () => {
      ...
      let lastAccessedIndex = parseInt(localStorage.getItem('lastAccessedIndex') ?? '0');

      if (!lastAccessedDate || new Date(lastAccessedDate).getDate() !== currentDate.getDate()) {
          lastAccessedIndex = (lastAccessedIndex + 1) % Object.keys(problems).length;
